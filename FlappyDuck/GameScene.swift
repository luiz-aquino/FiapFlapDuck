...
//  Copyright © 2017 br.com.luizaquino. All rights reserved.
//
        ...
        if !gameFinished {
            if !gameStarted {
                ...
                timer = Timer.scheduledTimer(withTimeInterval: 2.5, repeats: true, block: { (timer) in
                    ...
                    bottomPipe.position = CGPoint(x: self.size.width + upperPipe.size.width/2, y: upperPipe.position.y - upperPipe.size.height - pipesDistance)
                    bottomPipe.zPosition = 1
                    bottomPipe.physicsBody = SKPhysicsBody(rectangleOf: upperPipe.size)
